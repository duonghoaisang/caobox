...
if($_POST){
	...
	if($result->num_rows()){
		...
		cookie('remember', strlen($_SESSION['user_login']['id']).str_shuffle('iuy3748phs').$_SESSION['user_login']['id'].str_shuffle('abc4567hjk').$_SESSION['user_login']['password'].str_shuffle('12345678qwertyuighjklASDghj'));
		$hook->redirect('./');
	}else{
...
//$tpl->reset();
$tpl->setfile(array(
	'body'=>'user.forgot.tpl',
));
$error['web_title'] = 'Forgot password';
$tpl->assign($error);
