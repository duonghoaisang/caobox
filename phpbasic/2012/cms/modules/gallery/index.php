...
$cfg_type = array();
if($request['p']=='content' && $request['type']){
	$result = $dao->cfg_module(" `module`='product' AND typeid=".intval($request['type']));
}elseif($request['p']=='html'){
	$result = $dao->cfg_module(" `module`='html' AND page_or_id=".intval($request['pid']));
}
