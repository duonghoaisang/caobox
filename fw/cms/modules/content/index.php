...
$show_fields = array();
$request['by'] = isset($request['by'])?$request['by']:'';
$request['order'] = isset($request['order'])?$request['order']:'';
...
$level = 0;
if(in_array('newcat',$show_actions) && $request['parentid']){
	$aPath = $oCategory->xpath(intval($request['parentid']));
	$level = count($aPath);
