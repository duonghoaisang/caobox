...
if($request['module'] == 'content' || $request['module'] == 'html'){
	...
	while($rs = $cat->fetch()){
		...
		// category options
		$rs['checked'] = is_array($data['catoptions'])&&in_array($rs['typeid'],$data['catoptions'])?'checked':'';
		$rs['selected_'.$data['catoptions_type'][$rs['typeid']]] = 'selected';
		$tpl->assign($rs,'catoptions');
	}
}
...
// category file extra
/// file extra
if($data['catfile_extra']['name']){
	foreach($data['catfile_extra']['name'] as $k=>$name) if($name){
		$rs = array();
		$rs['name'] = $name;
		$rs['code'] = $data['catfile_extra']['code'][$k];
		$rs['type'] = $data['catfile_extra']['type'][$k];
		$rs['note'] = $data['catfile_extra']['note'][$k];
		$tpl->assign($rs,'catfile_extra');
