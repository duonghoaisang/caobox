function add_file_extra(sel,fieldname){
	var tr = '<tr><td><input type="text" name="'+fieldname+'[name][]" /></td><td><input name="'+fieldname+'[code][]" type="text" id="'+fieldname+'[code][]" value="" size="5" /></td><td><input type="text" name="'+fieldname+'[type][]" /></td><td><input type="text" name="'+fieldname+'[note][]" /><a href="#" onclick="delete_row(this);return false;"> x </a></td></tr>';
	$(sel).append(tr);
}

function add_fields_extra(){
	var tr = '<tr><td><input type="text" name="fields_extra[name][]" /></td><td><input name="fields_extra[code][]" type="text" id="fields_extra[code][]" value="" size="5" /></td><td><select name="fields_extra[type][]"><option value="input">Input</option><option value="textarea">Textarea</option><option value="tinymce">TextEditor</option></select></td><td><input type="text" name="fields_extra[note][]" /><a href="#" onclick="delete_row(this);return false;"> x </a></td></tr>';
	$('#fields_extra').append(tr);
}
function add_ln_fields_extra(){
	var tr = '<tr><td><input type="text" name="ln_fields_extra[name][]" /></td><td><input name="ln_fields_extra[code][]" type="text" id="ln_fields_extra[code][]" value="" size="5" /></td><td><select name="ln_fields_extra[type][]"><option value="input">Input</option><option value="textarea">Textarea</option><option value="tinymce">TextEditor</option></select></td><td><input type="text" name="ln_fields_extra[note][]" /><a href="#" onclick="delete_row(this);return false;"> x </a></td></tr>';
...
function fields_status(obj){
	if(obj.value == 'status'){
		$(obj).parent().find('p.info').show();	
	}else{
		$(obj).parent().find('p.info').hide();	
	}
}