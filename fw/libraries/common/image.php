...
class image extends Controller{
	...
	// public functions
	function resize($width = 0,$height = 0,$nFile = NULL,$force_resize = true,$specify = 'none'){
		$info = $this->info();
		...
		if($width <= 0){
			if(!$force_resize && $info[1] <= $height) return false; // 
			$width = round($info[0]*$height/$info[1]);
		}elseif($height<=0){
			if(!$force_resize && $info[0] <= $width) return false; // 
			$height = round($width*$info[1]/$info[0]);
		}else{ // width > 0 && height > 0
			if(!$force_resize && $info[0] <= $width && $info[1] <= $height) return false;
			if($specify == 'min'){
				if($info[0] <= $info[1]){
					$height = round($width*$info[1]/$info[0]);
				}else{
