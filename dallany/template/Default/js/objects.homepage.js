// JavaScript Document
var homepage = {sel: '#homepage', defaultTop: 400, introTop: 50, introH: 360, ourStoryTop: 50, ourStoryH: 410}
homepage.main = function(w,h){
	...
	$(sel).find('.intro').css({marginTop: defaultTop,height: 0,width: '100%'}).delay(2000).animate({marginTop: introTop},'slow',function(){
		$(this).animate({height: introH + 40},'slow').find('.copy').jScrollPane({showArrows:true});
		setperformance(1);
		//$(this).find('.copy').jScrollPane();
	});

}
homepage.insurance = function(){
	setperformance(0);
	var sel = this.sel;
	var defaultTop = this.defaultTop;
	...
	$(sel).find('.intro').animate({ width: 0},'fast',function(){
		$(this).hide();
		$(sel).find('.insurance').css({marginTop: defaultTop, height: 35}).fadeIn().animate({marginTop: ourStoryTop},'slow',function(){
			$(this).animate({height: ourStoryH + 40},'slow').find('.copy').jScrollPane({showArrows:true});
			setperformance(1);
		});	
