...
if($_POST){
	...
	foreach($cfg_type['main_fields'] as $code=>$info) if(isset($info['chose']) &&  $info['chose'] && $info['type'] == 'status'){
	...
	$cond = 1;
	$request['q'] = isset($request['q'])?$request['q']:'';
...
$request['msg'] = isset($request['msg'])?stripslashes($request['msg']):'';
//$tpl->assign($request);
$action = array();
