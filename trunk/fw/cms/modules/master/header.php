...
$request['parentid'] =  isset($request['parentid'])?intval($request['parentid']):0;
...
for($i=0;$i< $n;$i++) if($access=='ALL' || $access['act'][$i]){
	...
	foreach($MenuLink[$i] as $j=>$arr) if($access=='ALL' || $access['act'][$i][$j]){
		$addL = true;
		if(isset($arr['root_admin']) && $arr['root_admin'] && !$cfg['root_admin']) $addL = false;
		if($addL) $links .= '<a href="'.$arr['link'].'&menu='.$i.'.'.$j.'" class="left_'.(isset($arr['class'])&&$arr['class']?$arr['class']:'normal').($_SESSION['cms_menu']==$i.'.'.$j?' active':'').'" '.$class_hide.'>'.$arr['name'].'</a>';
	}
	$tpl->assign(array(
		'name'=>$MenuName[$i],
		'id'=>$i,
...
$tmp = $MenuLink[$menu[0]][$menu[1]];
$breadcrumb->assign($tmp['link'],$tmp['name']);
