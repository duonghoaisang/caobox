// AVIM 
if(typeof(fID)=='undefined'){fID=new Array();ifs=document.getElementsByTagName('IFRAME');if(ifs!=null){for(var ifsindex=0;ifsindex<ifs.length;ifsindex++){if(ifs[ifsindex].id.indexOf('_designEditor')!=-1){fID[fID.length]=ifs[ifsindex].id;}}}}
va="email".split(',')
method=0
on_off=1
dockspell=1
...
function statusMessage(){var str='phpbasic unicode: '
if(on_off==0)str+='Tắt'
else if(method==1)str+='TELEX';else if(method==2)str+='VNI';else if(method==0)str+='Tự động'
str+=" [F9] để thay đổi kiểu gõ, [F12] Bật/Tắt tiếng Việt"
window.status=str}
function updateInfo(){if(support)statusMessage()}
...
if((key==120)||(key==123)||(key==119)||(key==118)){if(key==120){on_off=1;setMethod(((method==2)?0:++method))}
...
function noCookie(){}
function doSetCookie(){var date=new Date();date.setTime(date.getTime()+(30*24*60*60*1000));var exp=date.toGMTString();document.cookie='mnHIMonoff='+on_off+';expires='+exp
document.cookie='mnHIMmethod='+method+';expires='+exp
document.cookie='mnHIMckspell='+dockspell+';expires='+exp
document.cookie='mnHIMdaucu='+dauCu+';expires='+exp}
function doGetCookie(){var ck=document.cookie,res=/HIM_method/.test(ck)
if((!res)||(ck.indexOf('mnHIMckspell')<0)){setCookie();return}
var p,ckA=ck.split(';')
for(var i=0;i<ckA.length;i++){p=ckA[i].split('=');p[0]=p[0].replace(/^\s+/g,"");p[1]=parseInt(p[1])
if(p[0]=='mnHIMonoff')on_off=p[1]
else if(p[0]=='mnHIMmethod')method=p[1]
else if(p[0]=='mnHIMckspell'){if(p[1]==0){dockspell=0;spellerr=nospell}
else{dockspell=1;spellerr=ckspell}}else if(p[0]=='mnHIMdaucu')dauCu=parseInt(p[1])}}
...
function init(){if((support)&&(!is_opera)&&(fID.length>0)){for(g=0;g<fID.length;g++){if(is_ie){frame=document.frames[fID[g]]
if((typeof(frame)!='undefined')&&(frame.document)){doc=frame.document;doc.onkeydown=function(){onKeyDown('iframe')}
doc.onkeypress=function(){FKeyPress();if(changed){changed=false;return false}}}}else{if(getEL(fID[g])){wi=getEL(fID[g]).contentWindow;iframedit=wi.document;iframedit.wi=wi;iframedit.addEventListener("keypress",ifMoz,false)
