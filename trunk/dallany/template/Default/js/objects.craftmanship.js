// JavaScript Document
var craftmanship = {sel: '#craftmanship' ,list: 'data.php?mod=xml&act=craftmanship',detail: 'data.php?mod=xml&act=craftmanship&id=', listH: 200, bgafter: 'bg-craftmanship-after.jpg'}
craftmanship.main = function(w,h){
	var sel = this.sel;
	var img = new Image;
	var list = this.list;
	...
	img.onload = function(){
		...
		$.post(list,{},function(data){
			...
			$('#background img.bgadvance').fadeIn(2000,function(){
				$(sel).show();
				$(sel).html(data);
				$(sel).append('<div class="overlapItem"></div>').find('.overlapItem').css({ left: 0, top: 0, width: '100%', height: h})
				$(sel).find('.list').css({right: w - (offset.left + 1024)});
				$(sel).find('.copy').css({height: listH}).jScrollPane({showArrows:true});
				$(sel).find('.viewer').nicescroll();
				setperformance(1);
...
craftmanship.showinfo = function(obj){
	thissrc=$(obj).find('img').attr("src");
	$('.list .title').find('img').fadeOut("slow",function(){
		$('.list .title').find('img').attr('src',thissrc);
		$('.list .title').find('img').fadeIn("slow");
	});
	/*
	var offset = $(obj).offset();
...
craftmanship.other_info = function(obj){
	var detail = this.detail;
	var href = obj.href.split('#')[1].split('/')[3];
...
craftmanship.closeinfo = function(){
	var sel = this.sel;
	$(sel).find('.overlapItem').fadeOut('slow',function(){$(this).remove();});
}


craftmanship.call_again = function(w,h){
	common.clear();
