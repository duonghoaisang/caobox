...
collection.main = function(w,h){
	...
	var itemW = Math.round((w - marginRight*num)/num)-2;
...
collection.listcat = function(obj){
		...
		$(sel).find('.fullimg').show().find('img').css({width: w});
		$(sel).find('.fullimg img.'+cls).show();
		//('fast',function(){
		$(sel).find('li div.overlap').animate({left: 0},'slow');
		$(sel).append('<p class="loading"></div>');
		$(sel).find('ul').fadeOut('fast');
		$(sel).find('.fullimg img.'+cls+'_after').fadeIn(4000,function(){
			$.post(list+href,{},function(data){
			//$(sel).find('ul').fadeOut('slow');
			//$(sel).find('.fullimg img.'+cls).animate({opacity: 0.8});
...
collection.changecat = function(obj){
	...
	$(sel).find('.overlapItem').fadeOut('slow',function(){
		...
		$(sel).find('ul').show();
		$(sel).find('.fullimg').show().find('img.'+obj.rel.replace('li','a')+'_after').css({width: w}).fadeIn('fast',function(){
