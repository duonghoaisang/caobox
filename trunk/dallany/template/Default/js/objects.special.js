// JavaScript Document
var special = {sel: '#special' ,list: 'data.php?mod=xml&act=special',detail: 'data.php?mod=xml&act=special&id=', listH: 444, bgafter: 'bg-special-after.jpg'}
special.main = function(w,h){
	var sel = this.sel;
	var img = new Image;
	var list = this.list;
	...
	img.onload = function(){
		...
		$.post(list,{},function(data){
			...
			$('#background img.bgadvance').fadeIn(2000,function(){
				...
				$(sel).find('.copy').css({height: listH});
...
special.other_info = function(obj){
	var detail = this.detail;
	var href = obj.href.split('#')[1].split('/')[3];
...
special.closeinfo = function(){
	var sel = this.sel;
	$(sel).find('.overlapItem').fadeOut('slow',function(){$(this).remove();});
}


special.call_again = function(w,h){
	common.clear();
