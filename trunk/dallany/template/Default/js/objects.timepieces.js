// JavaScript Document
var timepieces = {sel: '#timepieces' ,list: 'data.php?mod=xml&act=timepieces',detail: 'data.php?mod=xml&act=timepieces&id=', listH: 444, bgafter: 'bg-timepieces-after.jpg'}
timepieces.main = function(w,h){
	var sel = this.sel;
	var img = new Image;
	var list = this.list;
...
timepieces.showinfo = function(obj){
...
timepieces.other_info = function(obj){
	var detail = this.detail;
	var href = obj.href.split('#')[1].split('/')[3];
...
timepieces.closeinfo = function(){
	var sel = this.sel;
	$(sel).find('.overlapItem').fadeOut('slow',function(){$(this).remove();});
}


timepieces.call_again = function(w,h){
	common.clear();
